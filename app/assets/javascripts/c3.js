(function (global, factory) {
    ...
    c3_chart_internal_fn.redraw = function (options, transitions) {
        ...
        if ((withUpdateXDomain || withUpdateXAxis) && targetsToShow.length) {
            if (config.axis_x_tick_culling && tickValues) {
                ...
                intervalForCulling = 0;

                if (window.lineChartInterval)
                {
                    intervalForCulling = Number(window.lineChartInterval.toString());
                }

                if (intervalForCulling == 0) {
    ...
    c3_chart_internal_fn.parseDate = function (date) {
        ...
        if (!parsedDate || isNaN(+parsedDate)) {
            parsedDate = date;// =   window.console.error("Failed to parse x '" + date + "' to Date object");
        }
        return parsedDate;
    };
